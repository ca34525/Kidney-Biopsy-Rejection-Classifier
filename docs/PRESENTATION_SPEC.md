...
2. **Application:** Embed the existing research app. Use public screening specimen
   GSM6510425, then the incomplete example with IFNG removed. Show the score,
   threshold, flag and model version. The page must distinguish live requests
   from saved responses when used offline.
...
demonstrated one. The page checks the expected model version and threshold before
displaying the live app. Its saved response was generated by the same application
