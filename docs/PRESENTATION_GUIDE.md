...
The user's September 19, 2026 feedback sets the order and level of explanation. Open with only the title, **Classifying Kidney Transplant Rejection from Biopsy RNA**. Explain rejection and the biopsy with bold definition labels and evidence sub-bullets. Then explain why a molecular second opinion could help. The later September 21 wording pass replaces the paired hypothetical examples with one research-supported Example box. Follow the supporting research with the dataset, the outcome being classified, and the measurements used to predict it. Keep detailed assay terminology in the supporting documentation. Use ordinary bullets for parallel points and unbulleted accent lines for the requested summaries. Group explanations with their evidence. Substantive topics need visible support, but spoken elaboration and transitions do not each need a separate cue. These are the user's presentation preferences, separate from the external guidance summarized below.

The user's first September 21 instruction preserved slides 1–12 and replaced everything afterward, including the old backups. The later wording pass explicitly revises slides 2, 3, 4, 10, 11, and 12, superseding that preservation instruction for those slides. The 19-slide sequence and planned timings are unchanged. The report and application remain live demonstrations on slides 13–14. The rest of the technical section appears on slides: shared preparation and scoring, an interface other software can call, agreement with the saved analysis, and running and maintaining the application. Slide 19 closes on completed work. Select short code excerpts from the Code Guide and explain why they matter. Keep the full guide and detailed reports available for questions. Preserve all saved research and the demonstrated model. The first 12 slides retain their planned 11:25 allocation, followed by 8:35 for the replacement section. The [specification](PRESENTATION_SPEC.md) records the complete sequence; the [wording source note](references/PRESENTATION_WORDING_20260921.md) supports the clinical and CatBoost explanations.

The latest pass accepts three explanatory bullets under each measurement group on slide 7 and briefly clarifies the screening/validation terminology in slide 9's script. The slide 9 diagram is unchanged.
...
- On slide 3, open with "Sometimes the histology findings are ambiguous." Use one Example box: "A biopsy shows mild inflammation, but not enough to diagnose rejection. In a study of 146 borderline diagnoses, inflammation disappeared in some patients. Others later developed acute rejection." Do not imply a general prevalence. The second bullet is: "As a step towards potentially helping resolve ambiguous biopsies, I used RNA counts to classify recorded diagnoses. A high rejection score could add evidence for specialist review." Remove the continued-example box.
- On slide 4, explain both potential harms: missed rejection can leave kidney injury untreated, and unnecessary rejection treatment can worsen an infection. Keep that clinical rationale separate from any claim that this project's error tradeoff improves care; the [biopsy-care review](references/BIOPSY_CARE_20260919.md) and [wording source note](references/PRESENTATION_WORDING_20260921.md) record the evidence and the inference.
...
- Keep qualifications short and close to the claim they qualify. Give the limitation on slide 4 visual emphasis: "This project has not established usefulness in ambiguous biopsies. That requires comparing specialist assessment with and without RNA scores." Put the patient and referring-center separation note on slide 5, directly under the line with 1,193 transplant biopsies and 202 native-kidney controls. Make both lines bullets with identical font size and no asterisk. Keep slide 9 focused on the study split. Do not repeat obvious process statements or data QA results on slides whose purpose is to explain the data.
...
- Retain the detailed uncertainty and development-split evidence in saved reports. CatBoost's recall difference from all-RNA logistic regression was +4.73 percentage points, with a paired-bootstrap 95% interval of approximately −0.01 to +9.74 percentage points. The interval includes zero. The separate discovery-only follow-up selected CatBoost in 13 of 20 overlapping splits and all-RNA logistic regression in 7. Those counts do not establish a winner, and the follow-up did not change the demonstrated model. The main deck does not need to reteach these analyses.
...
- [ ] Slides 2, 3, 4, 10, 11, and 12 reflect the later September 21 wording requests; the remaining sequence and timings are unchanged. The technical section uses slides except for the report and application demonstrations.
