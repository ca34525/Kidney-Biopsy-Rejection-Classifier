...
The user's September 18, 2026 instruction puts all spoken text and delivery cues in a **separate HTML speaking script**. Do not use PowerPoint notes. The [current draft](../presentation/README.md) has 19 main slides, 8 backup slides, and a complete script planned for 20 minutes. Real timed rehearsals remain pending.
...
- Define biopsy and RNA in plain language before explaining the measurements. Keep assay, instrument, and panel terminology in backup slide 20. Give only the biological context needed to understand the data and label in the main talk.
...
- Keep qualifications short and close to the claim they qualify. Give the scope statement on slide 4 visual emphasis: "This project measures agreement with recorded diagnoses. Its value in ambiguous biopsies needs direct evaluation." Use an asterisk for the patient and referring-center separation qualification on slide 9. Do not repeat obvious process statements or data QA results on slides whose purpose is to explain the data.
...
- Preserve the current 19 main slides and 8 backup slides for this revision. The assay explanation is now the first backup. Use the freed time for the hypothetical example and error tradeoff. There is no universal bullets-per-slide formula.
