...
COPY_DIRS = ("src/kidney_biopsy", "scripts", "tests", "experiments",
             "docs", ".github", "data/demo")
COPY_FILES = ("README.md", "AGENTS.md", "pyproject.toml", "uv.lock", ".python-version",
              ".gitignore", "data/manifest.json")
...
def runtime_files(run_name: str) -> list[Path]:
    ...
    for item in manifest["artifacts"]:
        ...
        if (path.parent == run and path.suffix in {".json", ".csv"}
                and not any(word in path.name for word in
                            ("_test_predictions", "_screen_predictions", "biopsy_split", "raw_batch"))):
            verify(path, item)
            selected.append(path)
    for item in records():
        path = local_path(item["file"])
...
def clean_environment() -> dict[str, str]:
    ...
    for name in ("PYTHONPATH", "PYTHONHOME", "VIRTUAL_ENV", "UV_PROJECT_ENVIRONMENT",
                 "UV_PYTHON", "UV_CACHE_DIR", "MPLCONFIGDIR",
                 "KIDNEY_BIOPSY_PROJECT_ROOT", "KIDNEY_BIOPSY_RESULTS_DIR",
...
def run_step(command: list[str], root: Path, environment: dict, *, timeout: int = 600) -> dict:
    ...
    step = {"command": command, "returncode": completed.returncode,
...
def check_copy(run_name: str, output_name: str) -> None:
    ...
    cli = subprocess.run([sys.executable, "-m", "kidney_biopsy", "--counts-csv", sample_name,
                          "--results-dir", run_name, "--output", csv_output_name],
    ...
    with TestClient(create_app(project_root=ROOT, run_dir=run_name)) as client:
        ...
        if info["model_version"] != predictor.model_version or info["threshold"] != predictor.threshold:
            raise ValueError("API model metadata differs from the selected model.")
        if (info["required_targets"] != list(predictor.schema.required_targets)
                or not info["evaluation"]):
            raise ValueError("API is missing its assay contract or verified evaluation summary.")
        response = client.post("/predict", content=sample_path.read_bytes(),
        ...
        np.testing.assert_allclose(actual.rejection_score, expected.rejection_score, rtol=0, atol=1e-12)
        ...
        invalid_response = client.post("/predict", content=invalid,
                                       headers={"Content-Type": "text/csv"})
        invalid_body = invalid_response.json()
        if not (400 <= invalid_response.status_code < 500 and "error" in invalid_body
    ...
    summary = {
        ...
        "run_dir": run_name, "model_version": predictor.model_version,
        "environment": prefix.relative_to(ROOT).as_posix(),
        "package": package_path.relative_to(ROOT).as_posix(),
        "installed_static_assets": [name for name in ("index.html", "style.css", "app.js")
                                    if assets.joinpath(name).is_file()],
        "routes": pages, "specimens": len(expected), "numeric_tolerance": 1e-12,
        "evaluation_source": info["evaluation"]["source"],
        "max_absolute_score_difference": float(np.max(np.abs(actual.rejection_score - expected.rejection_score))),
    ...
    with output.open("x", encoding="utf-8") as stream:
        ...
        stream.write("\n")
    print(f"PASS: fresh installed application scored {len(expected)} specimens and rejected invalid input.")


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--run-dir", default="results/reproduction/20260915_shared")
    parser.add_argument("--check-dir", default=None, help="New folder under data/processed/setup_checks/.")
    ...
    record = {"verified_utc": datetime.now(timezone.utc).isoformat(), "successful": False,
              "copied_project": fresh_root.relative_to(ROOT).as_posix(),
              "run_dir": args.run_dir, "steps": [], "errors": [],
    ...
    try:
        ...
        commands = [
            ...
            ["uv", "run", "--no-sync", "python", "scripts/check_project.py", "--require-installed",
             "--output", "results/checks/fresh/checks.json"],
            ["uv", "run", "--no-sync", "python", "scripts/verify_fresh_setup.py", "--check-copy",
             "--run-dir", args.run_dir, "--output", "results/checks/fresh/application.json"],
        ]
        ...
        for key, name in (("software_checks", "checks.json"), ("application", "application.json")):
            record[key] = json.loads((fresh_root / "results/checks/fresh" / name).read_text(encoding="utf-8"))
