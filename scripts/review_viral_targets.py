...
if __package__:
    ...
    from .verify_local_data import sha256 as digest
else:
    from verify_local_data import local_path, records, verify
    from verify_local_data import sha256 as digest
...
COHORTS = {
    "Discovery cohort sample": "Discovery",
    "Validation cohort sample": "Author technical validation",
}
SOURCES = [
    {
        "title": "GSE212160 study deposit",
        "url": "https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE212160",
        "checked": "2026-09-15",
    },
    {
        "title": "Banff 2019 B-HOT panel consensus, Table 2",
        "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC7496585/",
        "checked": "2026-09-15",
    },
    {
        "title": "Zhang et al. Supplementary Table S3",
        "url": "https://ars.els-cdn.com/content/image/1-s2.0-S0023683723002477-mmc5.docx",
        "local_source": "data/reference/study/rejection_supplement_5.txt",
        "checked": "2026-09-15",
    },
    {
        "title": "Zhang et al. Supplementary Methods",
        "url": "https://ars.els-cdn.com/content/image/1-s2.0-S0023683723002477-mmc6.docx",
        "local_source": "data/reference/study/rejection_supplement_6.txt",
        "checked": "2026-09-15",
    },
]
            ...
            rows.append(
                {
                    **dict(zip(groups, keys)),
                    ...
                    "p25": signal.quantile(0.25),
                    "median": signal.median(),
                    "p75": signal.quantile(0.75),
...
def higher_signal_outcomes(frame: pd.DataFrame) -> dict[str, int]:
    ...
    predicted = frame.predicted.astype(
        bool
    )  # Joining discovery rows gives this column object dtype.
    return {
        "n": int(higher.sum()),
...
def main() -> None:
    ...
    baseline, out, specimen_dir = map(
        local_path, [args.baseline_dir, args.output_dir, args.specimen_dir]
    )
    ...
    if any(path.exists() for path in [out, specimen_dir]):
        parser.error(
            "Output destinations already exist; choose new output and specimen directories."
        )
    ...
    predictions = pd.read_csv(prediction_path, index_col="sample").rename(
        columns={"probability": "rejection_score"}
    )
    if (
        not np.isfinite(predictions.rejection_score.to_numpy(dtype=float)).all()
        or not predictions.rejection_score.between(0, 1).all()
        or predictions.predicted.dtype != bool
        or not np.isfinite(frozen["threshold"])
        or not 0 <= frozen["threshold"] <= 1
    ):
    ...
    if not np.array_equal(
        predictions.y, diagnosis.loc[validation_ids].ne("No Rejection").astype(int)
    ):
        raise ValueError("Prediction labels do not agree with source metadata.")
    if not np.array_equal(
        predictions.predicted, predictions.rejection_score.ge(frozen["threshold"])
    ):
    ...
    high = (
        frame.groupby(["cohort", "diagnosis"], observed=True)
        .both_BK_signals_above_zero.agg(n="size", both_above_zero="sum")
        .reset_index()
    )
    high["fraction"] = high.both_above_zero / high.n
    high_errors = (
        frame.loc[validation_ids]
        .groupby("error_group")
        .both_BK_signals_above_zero.agg(n="size", both_above_zero="sum")
        .reset_index()
    )
    ...
    for column in ["Date", "CartridgeID", "ScannerID"]:
        ...
        batch_rows.append(
            {
                "field": column,
                "discovery_distinct": len(discovery_values),
                "validation_distinct": len(validation_values),
                "shared_distinct": len(discovery_values & validation_values),
            }
        )
    ...
    metadata_inventory = [
        {
            "field": name,
            "origin": "GEO sample metadata",
            "n_present": int(meta[name].notna().sum()),
            "n_distinct": int(meta[name].nunique()),
            "values_if_at_most_10": sorted(map(str, meta[name].dropna().unique()))
            if meta[name].nunique() <= 10
            else [],
        }
    ...
    metadata_inventory += [
        {
            "field": name,
            "origin": "RCC header",
            "n_present": int(batches[name].notna().sum()),
            "n_distinct": int(batches[name].nunique()),
            "values_if_at_most_10": sorted(map(str, batches[name].dropna().unique()))
            if batches[name].nunique() <= 10
            else [],
        }
    ...
    write_csv(
        frame.rename_axis("specimen_id").reset_index(), specimen_dir / "viral_specimen_review.csv"
    )
    (out / "metadata_inventory.json").write_text(
        json.dumps(metadata_inventory, indent=2), encoding="utf-8"
    )
    ...
    for cohort in COHORTS.values():
        for histology in DIAGNOSES:
            values = cohorts.loc[
                cohorts.cohort.eq(cohort) & cohorts.diagnosis.eq(histology)
            ].set_index("target")
            tail = high.loc[high.cohort.eq(cohort) & high.diagnosis.eq(histology)].iloc[0]
            cells = [
                f"{values.loc[target, 'median']:.2f} ({values.loc[target, 'p25']:.2f} to {values.loc[target, 'p75']:.2f})"
                for target in TARGETS
            ]
            rows.append(
                [cohort, histology, str(int(tail.n)), *cells, str(int(tail.both_above_zero))]
            )
    error_rows = []
    for error_group, values in errors.groupby("error_group"):
        ...
        error_rows.append(
            [
                error_group,
                str(int(values.iloc[0].n)),
                *[f"{values.loc[target, 'median']:.2f}" for target in TARGETS],
            ]
        )
    ...
    nonrejection_discovery = int(
        high.loc[high.cohort.eq("Discovery") & high.diagnosis.eq("No Rejection"), "n"].iloc[0]
    )
    nonrejection_validation = int(
        high.loc[
            high.cohort.eq("Author technical validation") & high.diagnosis.eq("No Rejection"), "n"
        ].iloc[0]
    )
...
"""
    ...
    input_paths = [
        ...
        raw / "GSE212160_series_matrix.txt.gz",
        raw / "GSE212160_RAW.tar",
        split_path,
        prediction_path,
        frozen_path,
        importance_path,
    ...
    source_paths = [
        ...
        ROOT / "src/kidney_biopsy/source.py",
    ]
    ...
    reference_paths = [
        ROOT / "data/reference/study/rejection_source_manifest.json",
        ROOT / "data/reference/study/rejection_supplement_5.txt",
    ...
    optional_references = [
        {
            ...
            **(record(path) if path.is_file() else {}),
        }
    ...
    manifest = {
        ...
        "analysis": "descriptive follow-up; no fitting or threshold change",
        "selected_model": selected,
        "frozen_threshold": frozen["threshold"],
        "n_specimens": len(frame),
        "n_validation": len(validation_ids),
        "higher_signal_reference": "both normalized BK signals > 0; descriptive, not infection criterion",
        "inputs": [record(path) for path in input_paths],
        "optional_local_references": optional_references,
        "python": platform.python_version(),
        "dependencies": {name: version(name) for name in ["numpy", "pandas"]},
        "environment_lock": record(ROOT / "uv.lock"),
        "source_code": [record(path) for path in source_paths],
        "outputs": [
            record(path)
            for directory in [out, specimen_dir]
    ...
    print(
        f"Reviewed {len(frame)} specimens, including {len(validation_ids)} author-validation predictions. Report: {out.relative_to(ROOT).as_posix()}/REPORT.md"
    )
