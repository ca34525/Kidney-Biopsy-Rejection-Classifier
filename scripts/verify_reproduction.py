...
def main():
    ...
    for path in sorted(reference.glob('*_test_predictions.csv')):
        ...
        summaries.append({'file': path.name, 'specimens': len(old),
    ...
    old_split = pd.read_csv(reference / 'biopsy_split.csv').sort_values('sample').reset_index(drop=True)
    new_split = pd.read_csv(current / 'biopsy_split.csv').sort_values('sample').reset_index(drop=True)
    if not old_split.equals(new_split):
        raise ValueError('Actual specimen assignments differ.')
    result = {
        'verified_utc': datetime.now(timezone.utc).isoformat(),
        'reference_run': args.reference_dir, 'reproduced_run': args.run_dir,
        ...
        'split_specimens': len(old_split), 'score_tolerance': 1e-12,
        'selected_thresholds': thresholds, 'prediction_comparisons': summaries,
