...
def main():
    ...
    if output.exists():
        raise SystemExit("Demo output already exists; use a new --output-dir to preserve prepared files.")
    ...
    split_record = next(item for item in run_manifest["artifacts"]
    ...
    for ident, diagnosis in diagnoses:
        eligible = split.index[split["split"].eq("discovery_screen") & metadata.loc[split.index, "histology_diagnosis"].eq(diagnosis)]
        if len(eligible) == 0:
            raise ValueError(f"No screening example for {diagnosis}.")
        specimen = sorted(eligible)[0]
        selected.append((ident, diagnosis, specimen, counts.loc[[specimen], list(predictor.schema.required_targets)]))
    output.mkdir(parents=True)
    for ident, diagnosis, specimen, frame in selected:
        path = output / f"{ident}.csv"
        frame.to_csv(path, index_label="specimen")
        examples.append({
            "id": ident, "label": diagnosis, "description": f"Public discovery-screen specimen; recorded diagnosis: {diagnosis}.",
            "valid": True, "specimen": specimen, "recorded_diagnosis": diagnosis,
    ...
    examples.append({"id": "missing-target", "label": "Invalid: missing IFNG", "description": "The public no-rejection example with one required target removed.",
                     "valid": False, "file": path.relative_to(root).as_posix(), "bytes": path.stat().st_size, "sha256": sha256(path)})
    manifest = {
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "source_manifest": "data/manifest.json", "source_manifest_sha256": sha256(root / "data/manifest.json"),
        "source_files": source_manifest, "run_dir": args.results_dir,
        "model_version": predictor.model_version, "split_artifact": split_record,
