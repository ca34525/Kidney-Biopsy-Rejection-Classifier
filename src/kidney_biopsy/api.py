"""Small local research API; incoming counts stay in memory for one request."""
...
def _evaluation(root: Path, run_dir: str, predictor) -> dict | None:
    ...
    try:
        ...
        results = json.loads(verify_artifact(root, artifact).read_text(encoding="utf-8"))["any_rejection"]
...
def _demo_examples(root: Path, demo_dir: str) -> dict:
...
def create_app(*, project_root: str | Path | None = None, run_dir: str | None = None,
    ...
    async def lifespan(application: FastAPI):
        ...
        try:
            ...
            application.state.evaluation = _evaluation(root, selected_run, predictor)
            application.state.examples = _demo_examples(root, selected_demo)
        except Exception:
    ...
    application = FastAPI(
        title="Kidney biopsy rejection research API", version="0.1.0", lifespan=lifespan,
    ...
    def health():
        ...
        return JSONResponse({"status": "ready" if ready else "unavailable", "model_loaded": ready},
    ...
    def model():
        ...
        return {
            ...
            "threshold_selection": "Highest specificity retaining at least 90% of discovery-screen rejection cases.",
            "score_interpretation": "A model score, not a verified probability for an individual biopsy.",
            "evaluation": application.state.evaluation,
        }

    @application.post("/predict", openapi_extra={"requestBody": {
        "required": True, "content": {"text/csv": {"schema": {"type": "string"}}}}})
    async def predict(request: Request):
        predictor = application.state.predictor
        if predictor is None:
            return failure(503, "model_unavailable", "The configured research model is not ready.")
        if request.query_params:
            return failure(400, "unexpected_parameters", "This route accepts only a CSV body; it has no query parameters.")
        ...
        if "content-length" in request.headers:
            try:
                ...
                return failure(400, "invalid_content_length", "Content-Length must be a nonnegative integer.")
            if length < 0:
                return failure(400, "invalid_content_length", "Content-Length must be a nonnegative integer.")
        ...
        try:
            counts = await run_in_threadpool(read_counts_csv, io.StringIO(text), predictor.schema,
                                             max_specimens=MAX_SPECIMENS)
        except ValueError as error:
            # Validation contains no counts or specimen IDs. Bound even a long missing-target list.
            message = str(error)
            if message.startswith("Unexpected assay targets or metadata columns"):
                message = "Unexpected assay targets or metadata columns. Include only the required targets listed by /model."
            return failure(422, "invalid_csv", message[:700])
        try:
            result = await run_in_threadpool(predictor.predict, counts)
        except Exception:
            return failure(503, "prediction_unavailable", "The research model could not score this batch. No predictions were returned.")
        return {
            "predictions": result.to_dict(orient="records"),
            "input_checks": {"status": "passed", "specimens": len(counts),
                             "required_targets": len(predictor.schema.required_targets),
                             "batch_policy": "all_or_nothing", "assay_origin": "caller_responsibility"},
        }

    @application.get("/demo/examples")
    def examples():
        return {"examples": [{key: item[key] for key in ("id", "label", "description", "valid")}
    ...
    def example(example_id: str):
        ...
        try:
            ...
            return failure(503, "example_unavailable", "This public example failed its file check. Prepare the examples again.")
        return Response(body, media_type="text/csv", headers={"Content-Disposition": f'attachment; filename="{example_id}.csv"'})
