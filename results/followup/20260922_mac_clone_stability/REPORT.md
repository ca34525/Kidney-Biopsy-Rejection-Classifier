...
New destinations are required; completed runs are never overwritten. The command verifies both public input hashes before fitting. It uses the project lockfile and local Python 3.12 environment. The run took 242.6 seconds for 100 candidate fits; every saved candidate reproduced its screening scores within 1e-12 after reload.
...
- Per-specimen assignments/predictions and model binaries are local, Git-ignored artifacts under `data/processed/stability/20260922_mac_clone`. Model identities and all artifact hashes are recorded in the run manifest.
