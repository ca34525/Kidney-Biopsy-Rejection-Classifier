...
- Four-class logistic: **logistic_C0.01**, discovery macro F1 0.799; any-rejection threshold 0.747452902853.
...
For a clean checkout, run the documented download and binary-reproduction commands first. This follow-up uses the independently generated `results/reproduction/20260922_mac_clone` run and `data/processed/models/20260922_mac_clone` models. Populated local inputs need no download. Completed outputs are never overwritten: set fresh `--output-dir`, `--model-dir`, and `--case-dir` for another run. Use `--benchmark-run` and `--benchmark-models` to name another fixed reproduction.

This run fitted five candidates in 48.1 seconds including input verification, fitting, evaluation and chart preparation. `run_manifest.json` records exact elapsed time, package versions, input/output hashes, source snapshots and the lockfile used at run start. `frozen.json` was written before technical-validation predictions. New model save/reload scores agree within an absolute tolerance of 1e-12, with actual maximum differences in `verification.json`; benchmark scores also reproduce their saved prediction tables within that tolerance.
