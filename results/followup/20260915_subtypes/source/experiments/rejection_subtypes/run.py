...
from kidney_biopsy import AssaySchema, map_diagnoses, normalize_counts, predict_scores, read_geo_matrix, read_rcc_archive
...
LABELS = np.arange(4)
PLOT_NAMES = {"four_class_catboost": "Four-class CatBoost", "four_class_logistic": "Four-class logistic",
              "separate_components": "Separate components", "binary_benchmark": "Binary CatBoost"}
...
def ordered_class_scores(model, values: pd.DataFrame) -> np.ndarray:
    ...
    if (score.shape != (len(values), 4) or not np.isfinite(score).all()
...
def validate_binary_scores(y, score) -> tuple[np.ndarray, np.ndarray]:
    ...
    if y.ndim != 1 or len(y) == 0 or not np.isin(y, [0, 1]).all() or score.shape != y.shape:
        raise ValueError("Binary labels and scores must be matching, nonempty one-dimensional arrays.")
...
def threshold(y, score, sensitivity: float = .90) -> float:
    ...
    return float(max(candidate for candidate in np.unique(score)
                     if np.mean(score[y == 1] >= candidate) >= sensitivity))


def component_diagnoses(antibody_score, tcell_score, antibody_threshold: float, tcell_threshold: float) -> np.ndarray:
    ...
    return (antibody_score >= antibody_threshold).astype(int) + 2 * (tcell_score >= tcell_threshold).astype(int)


def validate_split(split: pd.DataFrame, metadata: pd.DataFrame) -> pd.DataFrame:
    if (split.empty or not split.index.is_unique or split.index.isna().any()
            or not metadata.index.is_unique or set(split.index) != set(metadata.index)):
    ...
    train, screen = train_test_split(discovery.to_numpy(), test_size=.25,
...
def wilson(successes: int, total: int) -> tuple[float | None, float | None]:
    ...
    width = z * np.sqrt(fraction * (1 - fraction) / total + z * z / (4 * total ** 2)) / (1 + z * z / total)
...
def binary_metrics(y, predicted, score=None, cutoff=None) -> dict:
    ...
    return {"n": len(y), "positives": tp + fn, "threshold": cutoff,
            "tp": tp, "fp": fp, "tn": tn, "fn": fn,
            "sensitivity": safe_ratio(tp, tp + fn), "specificity": safe_ratio(tn, tn + fp),
            "precision": safe_ratio(tp, tp + fp), "npv": safe_ratio(tn, tn + fn),
            "accuracy": float(np.mean(y == predicted)),
            "sensitivity_lower": sensitivity_lo, "sensitivity_upper": sensitivity_hi,
            "specificity_lower": specificity_lo, "specificity_upper": specificity_hi,
            "roc_auc": float(roc_auc_score(y, score)) if score is not None else None,
            "average_precision": float(average_precision_score(y, score)) if score is not None else None}
...
def subtype_metrics(y, predicted) -> tuple[dict, list[dict], np.ndarray]:
    ...
    for i, diagnosis in enumerate(DIAGNOSES):
        actual_n, predicted_n, correct = int(matrix[i].sum()), int(matrix[:, i].sum()), int(matrix[i, i])
        recall_lo, recall_hi = wilson(correct, actual_n)
        precision_lo, precision_hi = wilson(correct, predicted_n)
        classes.append({"diagnosis": diagnosis, "actual_n": actual_n, "predicted_n": predicted_n, "correct": correct,
                        "missed_class": actual_n - correct, "false_class": predicted_n - correct,
                        "sensitivity": safe_ratio(correct, actual_n), "precision": safe_ratio(correct, predicted_n),
                        "sensitivity_lower": recall_lo, "sensitivity_upper": recall_hi,
                        "precision_lower": precision_lo, "precision_upper": precision_hi})
    summary = {"n": len(y), "accuracy": float(np.mean(np.asarray(y) == predicted)),
               "macro_f1": float(f1_score(y, predicted, labels=LABELS, average="macro", zero_division=0)),
               "correct": int(np.trace(matrix)), "incorrect": int(matrix.sum() - np.trace(matrix))}
    return summary, classes, matrix


def marginal_scores(score: np.ndarray) -> dict[str, np.ndarray]:
    return {"any_rejection": score[:, 1:].sum(axis=1),
...
def binary_targets(codes: pd.Series) -> dict[str, pd.Series]:
    return {"any_rejection": codes.ne(0).astype(int),
            "antibody_mediated_component": codes.isin([1, 3]).astype(int),
            "t_cell_mediated_component": codes.isin([2, 3]).astype(int)}


def candidates() -> dict:
    result = {f"catboost_depth{depth}": CatBoostClassifier(
        iterations=300, depth=depth, learning_rate=.04, loss_function="MultiClass",
        random_seed=2026, thread_count=3, verbose=False, allow_writing_files=False) for depth in [4, 6]}
    for c in [.01, .1, 1.]:
        result[f"logistic_C{c:g}"] = make_pipeline(StandardScaler(), LogisticRegression(C=c, solver="lbfgs", max_iter=3000))
    return result
...
def plot_results(out: Path, confusion: pd.DataFrame, any_metrics: pd.DataFrame, mixed: pd.DataFrame) -> None:
    ...
    for ax, model in zip(axes, validation.model.unique()):
        table = validation[validation.model.eq(model)].pivot(index="actual", columns="predicted", values="n").reindex(index=DIAGNOSES, columns=DIAGNOSES)
        matrix = table.to_numpy()
        ax.imshow(matrix, cmap="Blues", vmin=0, vmax=176)
        for i in range(4):
            for j in range(4):
                ax.text(j, i, str(matrix[i, j]), ha="center", va="center", color="white" if matrix[i, j] > 100 else "#18242E")
        ax.set(xticks=LABELS, yticks=LABELS, xticklabels=["None", "Antibody", "T-cell", "Mixed"],
               yticklabels=["None", "Antibody", "T-cell", "Mixed"], xlabel="Predicted diagnosis", ylabel="Recorded diagnosis", title=PLOT_NAMES[model])
    ...
    for offset, column, label, color in [(-.19, "fn", "Missed rejection / 169", "#B55635"), (.19, "fp", "False flags / 176", "#247C96")]:
        bars = ax.bar(x + offset, rows[column], width=.36, label=label, color=color)
        ax.bar_label(bars, padding=3)
    ax.set(xticks=x, xticklabels=[PLOT_NAMES[name] for name in rows.model], ylabel="Specimens", ylim=(0, float(rows[["fn", "fp"]].max().max()) + 8),
    ...
    for column, label, color in zip(["as_no_rejection", "as_antibody", "as_tcell", "as_mixed"], SHORT_NAMES, ["#AAB4BC", "#257F9F", "#A16BBD", "#347C5C"]):
        values = rows[column].to_numpy()
        bars = ax.bar(x, values, bottom=bottom, width=.6, label=label, color=color)
        for bar, value, start in zip(bars, values, bottom):
            if value:
                ax.text(bar.get_x() + bar.get_width() / 2, start + value / 2, str(value), ha="center", va="center", color="white" if column != "as_no_rejection" else "black")
        bottom += values
    ax.set(xticks=x, xticklabels=[PLOT_NAMES[name] for name in rows.model], ylabel="Specimens with recorded mixed rejection", ylim=(0, 22),
...
def report(out: Path, configuration: dict, frozen: dict, tables: dict[str, pd.DataFrame], elapsed: float) -> None:
    def percent(value):
    ...
    any_rows = tables["any_rejection_metrics"].query("split == 'author_validation'").set_index("model")
    ...
    lines = ["# Rejection subtype follow-up", "", "## Main result", "",
    ...
    for name in summaries.index:
        lines.append(f"| {PLOT_NAMES[name]} | {int(summaries.loc[name, 'correct'])} | {summaries.loc[name, 'macro_f1']:.3f} | {int(mixed.loc[name, 'as_mixed'])} | {int(any_rows.loc[name, 'fn'])} | {int(any_rows.loc[name, 'fp'])} |")
    binary = any_rows.loc["binary_benchmark"]
    lines.extend([f"| Binary CatBoost benchmark | — | — | — | {int(binary.fn)} | {int(binary.fp)} |", "",
    ...
    for family, item in frozen["four_class_models"].items():
        lines.append(f"- {PLOT_NAMES[family]}: **{item['candidate']}**, discovery macro F1 {item['screen']['macro_f1']:.3f}; any-rejection threshold {item['thresholds']['any_rejection']:.12g}.")
    lines.extend(["", "Each sum-of-rejection threshold is the highest screening score that retains at least 90% of screening rejection cases, matching the original binary operating rule. "
    ...
    for split in SPLITS:
        ...
        lines.append(f"| {split} | " + " | ".join(str(int(row[name])) for name in DIAGNOSES) + f" | {int(row.sum())} |")
    lines.extend(["", "The raw GSE212160 RCC measurements are read through the shared source reader. Each specimen is transformed by log2(count + 1), "
    ...
    for row in classes.itertuples(index=False):
        lines.append(f"| {PLOT_NAMES[row.model]} | {row.diagnosis} | {row.correct}/{row.actual_n} | {row.predicted_n} | {percent(row.sensitivity)} ({percent(row.sensitivity_lower)}–{percent(row.sensitivity_upper)}) | {percent(row.precision)} ({percent(row.precision_lower)}–{percent(row.precision_upper)}) |")
    lines.extend(["", "All four-by-four counts, including zeros, are in `confusion_matrices.csv` and the [editable confusion chart](figures/01_subtype_confusion.svg).", "",
                  "## Mixed rejection", "",
                  "| Model | Predicted none | Predicted antibody only | Predicted T-cell only | Predicted mixed |", "| --- | ---: | ---: | ---: | ---: |"])
    for name, row in mixed.iterrows():
        lines.append(f"| {PLOT_NAMES[name]} | {int(row.as_no_rejection)} | {int(row.as_antibody)} | {int(row.as_tcell)} | {int(row.as_mixed)} |")
    lines.extend(["", "The separate-component prediction is mixed only when both preserved component flags are positive; antibody only and T-cell only require the corresponding flag alone. "
    ...
    for name, row in any_rows.iterrows():
        ...
        lines.append(f"| {PLOT_NAMES[name]} | {cutoff} | {percent(row.sensitivity)} | {percent(row.specificity)} | {int(row.fn)} | {int(row.fp)} | {int(row.fn-binary.fn):+d} | {int(row.fp-binary.fp):+d} |")
    lines.extend(["", "The 90% sensitivity setting applied to discovery screening; it does not guarantee 90% sensitivity in technical validation. "
                  ...
                  "## Reproduce and inspect", "", "From the project root:", "", "```powershell", "uv sync --locked",
                  "uv run python scripts/verify_local_data.py", "uv run python experiments/rejection_subtypes/run.py", "```", "",
...
def main() -> None:
    ...
    benchmark, benchmark_models, out, model_dir, case_dir = [local_path(value) for value in
        [args.benchmark_run, args.benchmark_models, args.output_dir, args.model_dir, args.case_dir]]
    ...
    configuration = {"analysis_type": "Follow-up; technical-validation results were already examined in prior work.",
        "benchmark_run": args.benchmark_run, "benchmark_models": args.benchmark_models,
        "output_dir": args.output_dir, "model_dir": args.model_dir, "case_dir": args.case_dir,
        "diagnosis_order": list(DIAGNOSES), "discovery_split_seed": 20260915,
        ...
        "catboost_grid": {"depth": [4, 6], "iterations": 300, "learning_rate": .04, "loss_function": "MultiClass", "random_seed": 2026, "thread_count": 3},
        "logistic_grid": {"C": [.01, .1, 1.], "solver": "lbfgs", "max_iter": 3000, "scaler": "StandardScaler fitted on training only", "loss": "multinomial", "penalty": "L2"},
        ...
        "bootstrap": {"replicates": 2000, "seed": 20260915, "unit": "biopsy specimen", "scheme": "paired nonparametric percentile"}}
    ...
    source_paths = [Path(__file__), ROOT / "scripts/verify_local_data.py", ROOT / "tests/test_subtypes.py", ROOT / "pyproject.toml", ROOT / "uv.lock",
                    *[ROOT / "src/kidney_biopsy" / name for name in ["__init__.py", "preprocessing.py", "source.py", "prediction.py"]]]
    snapshots = []
    for path in source_paths:
        destination = out / "source" / path.relative_to(ROOT)
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(path, destination)
        snapshots.append({"file": relative(path), "snapshot": relative(destination), "sha256": sha256(destination)})
    raw = ROOT / "data/raw/rejection_public"
    _, metadata = read_geo_matrix(raw / "GSE212160_series_matrix.txt.gz")
    split = validate_split(pd.read_csv(benchmark / "biopsy_split.csv", index_col="sample"), metadata)
    ...
    if pd.DataFrame({"hash": profile_hash, "split": split.split}).groupby("hash").split.nunique().gt(1).any():
    ...
    class_counts = pd.crosstab(split.split, split.histology).reindex(index=SPLITS, columns=DIAGNOSES).stack().rename("n").reset_index().rename(columns={"histology": "diagnosis"})
    write_json(out / "data_audit.json", {"specimens": len(features), "excluded": 0, "features": len(features.columns),
        "duplicates": int(features.duplicated(keep=False).sum()), "cross_split_duplicate_profiles": 0,
        "split_matches_saved_and_recomputed": True, "source_labels_match_saved_split": True,
        "normalization_schema_matches_all_benchmarks": True, "class_counts": class_counts.to_dict(orient="records")})
    ...
    for name, model in all_candidates.items():
        ...
        summary, _, _ = subtype_metrics(codes.loc[ids["discovery_screen"]].to_numpy(), score.argmax(axis=1))
        screening = {"candidate": name, "family": "four_class_catboost" if name.startswith("catboost") else "four_class_logistic",
                     **summary, "seconds": time.perf_counter() - fit_start}
        screens.append(screening)
        print("SCREEN", json.dumps(screening), flush=True)
    frozen = {"written_utc": datetime.now(timezone.utc).isoformat(), "four_class_models": {},
              "benchmark_run": args.benchmark_run, "benchmark_models": benchmark_frozen,
              "selection": configuration["selection"], "schema": schema.to_dict(), "class_order": list(DIAGNOSES)}
    for family in ["four_class_catboost", "four_class_logistic"]:
        ...
        selected = max([row for row in screens if row["family"] == family], key=lambda row: (row["macro_f1"], row["accuracy"]))
        name = selected["candidate"]
        cutoffs = {target: threshold(targets[target].loc[ids["discovery_screen"]], score)
        ...
        frozen["four_class_models"][family] = {"candidate": name, "thresholds": cutoffs, "screen": selected,
            "model_file": relative(path), "model_sha256": sha256(path), "model_version": f"{out.name}:{name}"}
    ...
    print("FROZEN", json.dumps({family: item["candidate"] for family, item in frozen["four_class_models"].items()}), flush=True)

    # All model choices and thresholds are now fixed; begin validation scoring.
    benchmark_models_loaded = {target: joblib.load(benchmark_models / f"{target}_selected_model.joblib") for target in TARGETS}
    selected_models = {family: joblib.load(local_path(item["model_file"])) for family, item in frozen["four_class_models"].items()}
    verification = {"frozen_before_validation_scores": True, "frozen_sha256": frozen_hash, "score_absolute_tolerance": 1e-12,
                    "model_reload_max_absolute_difference": {}, "benchmark_saved_score_max_absolute_difference": {}}
    ...
    for split_name in ["discovery_screen", "author_validation"]:
        ...
        case = pd.DataFrame({"histology": split.loc[index, "histology"], "diagnosis_code": truth}, index=index)
        four_scores = {}
        four_predictions = {}
        any_predictions = {}
        for family, model in selected_models.items():
            score = ordered_class_scores(model, features.loc[index])
            original = ordered_class_scores(all_candidates[frozen["four_class_models"][family]["candidate"]], features.loc[index])
            ...
            for target, marginal in marginal_scores(score).items():
                ...
                result = {"split": split_name, "model": family, "target": target,
        ...
        for target, model in benchmark_models_loaded.items():
            ...
            result = {"split": split_name, "model": name, "target": target,
            ...
            saved_path = benchmark / f"{target}_{benchmark_frozen[target]['selected_model']}_{suffix}_predictions.csv"
            ...
            if not np.array_equal(saved.y, targets[target].loc[index]) or not np.allclose(saved[score_column], score, rtol=0, atol=1e-12):
                raise ValueError("Benchmark scores or labels differ from the preserved run.")
            verification["benchmark_saved_score_max_absolute_difference"][f"{split_name}:{target}"] = maximum
        combined = component_diagnoses(component_scores["antibody_mediated_component"], component_scores["t_cell_mediated_component"],
                                       benchmark_frozen["antibody_mediated_component"]["threshold"], benchmark_frozen["t_cell_mediated_component"]["threshold"])
        ...
        any_rows.append({"split": split_name, "model": "separate_components", "target": "any_rejection",
        ...
        for model, prediction in four_predictions.items():
            ...
            confusion_rows.extend({"split": split_name, "model": model, "actual": DIAGNOSES[i], "predicted": DIAGNOSES[j], "n": int(matrix[i, j])}
                                  for i in range(4) for j in range(4))
            mixed_rows.append({"split": split_name, "model": model, "actual_mixed": int(matrix[3].sum()),
                               **{key: int(matrix[3, i]) for i, key in enumerate(["as_no_rejection", "as_antibody", "as_tcell", "as_mixed"])}})
        for model, prediction in any_predictions.items():
            for i, diagnosis in enumerate(DIAGNOSES):
                mask = truth == i
                error_rows.append({"split": split_name, "model": model, "diagnosis": diagnosis, "n": int(mask.sum()),
                                   "errors": int(np.sum(prediction[mask] != (i != 0))), "error_type": "false_flag" if i == 0 else "missed_rejection"})
    ...
    for name, flags in validation_flags.items():
        if name == "binary_benchmark":
            continue
        for metric, target_value, error_value in [("missed_rejection", 1, False), ("false_flags", 0, True)]:
            difference = ((truth == target_value) & (flags == error_value)).astype(int) - ((truth == target_value) & (baseline == error_value)).astype(int)
            draws = difference[bootstrap_indices].sum(axis=1)
            lower, upper = np.quantile(draws, [.025, .975])
            difference_rows.append({"model": name, "reference": "binary_benchmark", "metric": metric, "estimate": int(difference.sum()),
                                    "lower": float(lower), "upper": float(upper), "resamples": 2000})
    tables = {"class_counts": class_counts, "subtype_summary": pd.DataFrame(summary_rows), "per_class_metrics": pd.DataFrame(class_rows),
              "confusion_matrices": pd.DataFrame(confusion_rows), "any_rejection_metrics": pd.DataFrame(any_rows),
              "component_metrics": pd.DataFrame(component_rows), "mixed_rejection": pd.DataFrame(mixed_rows),
              "any_errors_by_diagnosis": pd.DataFrame(error_rows), "paired_error_differences": pd.DataFrame(difference_rows)}
    for name, table in tables.items():
        table.to_csv(out / f"{name}.csv", index=False)
    plot_results(out, tables["confusion_matrices"], tables["any_rejection_metrics"], tables["mixed_rejection"])
    elapsed = time.perf_counter() - start
    report(out, configuration, frozen, tables, elapsed)
    artifacts = [{"file": relative(path), "bytes": path.stat().st_size, "sha256": sha256(path)}
                 for directory in [out, model_dir, case_dir] for path in sorted(directory.rglob("*")) if path.is_file()]
    write_json(out / "run_manifest.json", {"analysis_type": configuration["analysis_type"], "started_utc": started,
        "finished_utc": datetime.now(timezone.utc).isoformat(), "elapsed_seconds": time.perf_counter() - start,
        "python": platform.python_version(), "dependencies": {name: version(name) for name in ["catboost", "joblib", "numpy", "pandas", "scikit-learn", "scipy", "matplotlib"]},
        "sources": snapshots, "raw_sources": raw_records, "inputs": [{"file": relative(path), "sha256": sha256(path)} for path in input_paths],
        "input_manifest_sha256": sha256(ROOT / "data/manifest.json"), "fit_count": 5,
        "environment_lock": next(row for row in snapshots if row["file"] == "uv.lock"), "artifacts": artifacts})
    print(f"COMPLETE: {relative(out / 'REPORT.md')}; {time.perf_counter() - start:.1f} seconds", flush=True)
