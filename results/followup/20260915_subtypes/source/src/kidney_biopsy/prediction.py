...
def _check_model_features(model: Any, features) -> None:
    ...
    if names is None or list(names) != expected:
        raise ValueError("Model feature names or order do not match the frozen assay schema.")
    count = getattr(model, "n_features_in_", len(expected))
    ...
    if count not in (0, len(expected)):
...
def predict_scores(model: Any, normalized_features: pd.DataFrame) -> np.ndarray:
    ...
    try:
        values = normalized_features.to_numpy(dtype=float)
    ...
    predictions = np.asarray(model.predict_proba(normalized_features), dtype=float)
    if (predictions.shape != (len(normalized_features), 2)
            or not np.isfinite(predictions).all()
            or (predictions < 0).any() or (predictions > 1).any()
            or not np.allclose(predictions.sum(axis=1), 1, rtol=0, atol=1e-7)):
...
class Predictor:
    ...
    def __post_init__(self):
        ...
        if self.target != "any_rejection" or not isinstance(self.model_version, str) or not self.model_version.strip():
    ...
    def predict(self, counts: pd.DataFrame) -> pd.DataFrame:
        ...
        return self.predict_normalized(normalize_counts(counts, self.schema))
...
def load_predictor(project_root: str | Path, run_dir: str,
                   model_dir: str | None = None) -> Predictor:
...
"""
    root = Path(project_root).resolve()
    results = project_path(root, run_dir)
    manifest = json.loads((results / "run_manifest.json").read_text(encoding="utf-8"))
    configured_model_dir = manifest["model_dir"]
    if model_dir is not None and project_path(root, model_dir) != project_path(root, configured_model_dir):
    ...
    frozen_path = results / "any_rejection_frozen.json"
    ...
    for path in (frozen_path, model_path):
        ...
        verify_artifact(root, artifacts[relative])
    ...
    model_version = frozen.get("model_version") or f"{frozen['selected_model']}-{sha256(model_path)[:12]}"
    return Predictor(model, schema, float(frozen["threshold"]), model_version, frozen["target"])
