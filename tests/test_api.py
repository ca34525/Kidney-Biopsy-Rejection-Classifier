...
from kidney_biopsy.preprocessing import AssaySchema, HOUSEKEEPING_TARGETS, normalize_counts
...
class ApiTests(unittest.TestCase):
    def setUp(self):
        ...
        self.raw = pd.DataFrame({"IFNG": [0., 10., 50., 100.], "CXCL9": [1., 8., 80., 90.],
                                 **{target: [1., 2., 3., 4.] for target in HOUSEKEEPING_TARGETS}},
        ...
        self.frozen = {"features": list(self.schema.features), "schema": self.schema.to_dict(),
                       "threshold": .61, "target": "any_rejection", "selected_model": "logistic",
    ...
    def post(self, frame):
        return self.client.post("/predict", content=frame.to_csv(index_label="specimen"),
    ...
    def test_api_cli_and_direct_scores_agree_with_reordered_columns(self):
        ...
        with redirect_stdout(io.StringIO()):
            predict_main(["--project-root", str(self.root), "--results-dir", "results/test",
                          "--counts-csv", "counts.csv", "--output", "cli.csv"])
        before_files = sorted(path.relative_to(self.root) for path in self.root.rglob("*") if path.is_file())
        ...
        assert_frame_equal(actual, load_predictor(self.root, "results/test").predict(self.raw), atol=1e-12, rtol=0)
        self.assertEqual(actual.rejection_flag.tolist(), (actual.rejection_score >= .61).tolist())
        self.assertEqual(response.json()["input_checks"]["specimens"], 4)
        self.assertEqual(response.headers["cache-control"], "no-store")
        self.assertEqual(sorted(path.relative_to(self.root) for path in self.root.rglob("*") if path.is_file()), before_files)

    def test_missing_duplicate_or_extra_targets_fail_entire_batch(self):
        duplicate = pd.concat([self.raw, self.raw[["IFNG"]]], axis=1)
        for data in [self.raw.drop(columns="IFNG"), self.raw.drop(columns=HOUSEKEEPING_TARGETS[0]),
                     duplicate, self.raw.assign(private_metadata="private-value")]:
    ...
    def test_one_invalid_count_rejects_every_specimen_without_echoing_data(self):
        for value in ["private-count-value", "", -1., np.nan, np.inf]:
    ...
    def test_empty_ragged_quoted_and_non_utf8_csv_fail_clearly(self):
        for body in [b"", b"specimen,IFNG\n", b"specimen,IFNG\na,1,2\n", b'specimen,IFNG\n"unclosed,2', b"\xff\xfe"]:
            with self.subTest(body=body):
                self.assert_invalid(self.client.post("/predict", content=body, headers={"Content-Type": "text/csv"}))

    def test_batch_limit_is_applied_before_returning_results(self):
        frame = pd.concat([self.raw.iloc[[0]]] * (MAX_SPECIMENS + 1)).set_axis([str(i) for i in range(MAX_SPECIMENS + 1)])
        response = self.post(frame)
        self.assert_invalid(response)
    ...
    def test_body_size_limit_checks_announced_and_actual_bytes(self):
        ...
        for headers in [{"Content-Type": "text/csv"}, {"Content-Type": "text/csv", "Content-Length": "1"}]:
        ...
        request = self.client.build_request("POST", "/predict", content=iter([body[:100], body[100:]]),
                                            headers={"Content-Type": "text/csv"})
    ...
    def test_wrong_media_encoding_and_content_length_fail(self):
        for headers, status in [({"Content-Type": "application/json"}, 415),
            ...
            with self.subTest(headers=headers):
                self.assert_invalid(self.client.post("/predict", content=b"counts", headers=headers), status)
    ...
    def test_model_is_loaded_once_and_caller_cannot_select_an_artifact(self):
        with patch("kidney_biopsy.api.load_predictor", wraps=load_predictor) as loader:
            with TestClient(create_app(project_root=self.root, run_dir="results/test")) as client:
                self.assertEqual(client.get("/health").json(), {"status": "ready", "model_loaded": True})
                schema = client.get("/model").json()
                self.assertEqual(schema["required_targets"], list(self.schema.required_targets))
                self.assertEqual(schema["threshold"], .61)
                response = client.post("/predict?model_dir=uploaded-model", content=self.raw.to_csv(index_label="specimen"),
                                       headers={"Content-Type": "text/csv"})
    ...
    def test_unexpected_prediction_failure_has_no_partial_results_or_internal_details(self):
        with patch.object(self.app.state.predictor.model, "predict_proba", side_effect=RuntimeError("private internal values")):
