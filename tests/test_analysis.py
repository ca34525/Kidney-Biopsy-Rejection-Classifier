...
class AnalysisValidationTests(unittest.TestCase):
    def setUp(self):
        self.reference = pd.DataFrame({"histology": ["No Rejection", "Mixed Rejection"]}, index=["a", "b"])
        self.predictions = pd.DataFrame({"y": [0, 1], "probability": [.2, .8], "predicted": ["False", "True"]}, index=["a", "b"])

    def test_prediction_join_matches_names_not_row_positions(self):
        result = align_predictions(self.predictions.iloc[::-1], self.reference, .5)
        np.testing.assert_array_equal(result.probability, [.2, .8])
        np.testing.assert_array_equal(result.predicted, [False, True])

    def test_unequal_and_duplicate_specimen_sets_fail(self):
        bad = [self.predictions.iloc[:1], self.predictions.rename(index={"b": "c"}),
        ...
        for frame in bad:
            with self.subTest(index=frame.index.tolist()), self.assertRaises(ValueError):
                align_predictions(frame, self.reference, .5)

    def test_bad_labels_scores_and_frozen_decisions_fail(self):
        for column, value in [("y", 1), ("probability", np.nan), ("probability", np.inf),
                              ("probability", -.1), ("probability", 1.1), ("predicted", "maybe"), ("predicted", "True")]:
            bad = self.predictions.copy()
            bad.loc["a", column] = value
            with self.subTest(column=column, value=value), self.assertRaises(ValueError):
                align_predictions(bad, self.reference, .5)
...
class UncertaintyTests(unittest.TestCase):
    def test_all_positive_constant_has_undefined_npv(self):
        row = metrics(np.array([0, 0, 1, 1]), np.full(4, .7), .5)
        self.assertTrue(np.isnan(row["npv"]))
        self.assertEqual((row["fn"], row["fp"], row["roc_auc"]), (0, 2, .5))

    def test_reliability_keeps_empty_bins_and_includes_one(self):
        rows = reliability(np.array([0, 1, 0, 1]), np.array([0, .1, .99, 1]), "model")
        self.assertEqual(len(rows), 10)
        self.assertEqual([row["n"] for row in rows], [1, 1, 0, 0, 0, 0, 0, 0, 0, 2])
        self.assertEqual(rows[-1]["observed_fraction"], .5)
    ...
    def test_pairing_preserves_identical_model_differences_and_seed(self):
        y = np.tile([0, 1], 10)
        score = np.linspace(.02, .98, len(y))
        values = {"selected": score, "comparison": score.copy(), "training_prevalence": np.full(len(y), .7)}
        thresholds = {key: .5 for key in values}
        ...
        row = intervals.loc[(intervals.model == "training_prevalence") & (intervals.metric == "npv")].iloc[0]
